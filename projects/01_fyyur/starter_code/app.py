...
def create_venue_submission():
  ...
  try:
    ...
    db.session.close()
  if error:
   flash('Error in creating venue for ' + request.fotm['name'])
  else:
...
def create_artist_submission():
  ...
  try:
    ...
    body = Artist(name = name, city = city, state = state, phone = phone, genres = genres, image_link = image_link , website_link = website_link , seeking_venue = seeking_venue, seeking_description)
    db.session.add(body)
    db.session.commit()
    data['name'] = body.name
    data['city'] = body.city
